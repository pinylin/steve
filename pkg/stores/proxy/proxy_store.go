...
func (s *Store) list(apiOp *types.APIRequest, schema *types.APISchema, client dynamic.ResourceInterface) (types.APIObjectList, error) {
	logrus.Infof("proxy_store list:1, type: %s, flag: %s, time: %s", apiOp.Request.URL, apiOp.Request.Header.Get("flag"), time.Now().String())
	...
	if revision := parseRevision(apiOp); revision != "" {
		logrus.Infof("proxy_store list getCache:1, type: %s, flag: %s, time: %s", apiOp.Request.URL, apiOp.Request.Header.Get("flag"), time.Now().String())
		list, err := s.getCache(getCacheKey(apiOp.Request.URL.Path, revision, apiOp.Namespace, opts.Continue))
		logrus.Infof("proxy_store list getCache:2, type: %s, flag: %s, time: %s", apiOp.Request.URL, apiOp.Request.Header.Get("flag"), time.Now().String())
		if err != nil {
		...
		if list != nil {
			logrus.Infof("proxy_store list tableToList:1, type: %s, flag: %s, time: %s", apiOp.Request.URL, apiOp.Request.Header.Get("flag"), time.Now().String())
			tableToList(list)
			logrus.Infof("proxy_store list tableToList:2, type: %s,  flag: %s, time: %s", apiOp.Request.URL, apiOp.Request.Header.Get("flag"), time.Now().String())
	...
	logrus.Infof("proxy_store list:2, type: %s, flag: %s, time: %s", apiOp.Request.URL, apiOp.Request.Header.Get("flag"), time.Now().String())
	...
	logrus.Infof("proxy_store list:3, type: %s, flag: %s, time: %s", apiOp.Request.URL, apiOp.Request.Header.Get("flag"), time.Now().String())
	for i := range resultList.Items {
		result.Objects = append(result.Objects, toAPI(schema, (&resultList.Items[i]).DeepCopy()))
	}
	logrus.Infof("proxy_store list:4, type: %s, flag: %s, time: %s", apiOp.Request.URL, apiOp.Request.Header.Get("flag"), time.Now().String())
	return result, nil
}
